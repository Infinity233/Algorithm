public class RBTree<T extends Comparable<T>> {
    ...
    private RBNode<T> parentOf(RBNode<T> node) { //获得父节点
        return node != null ? node.parent : null;
    }

    private void setParent(RBNode<T> node, RBNode<T> parent) { //设置父节点
        if (node != null)
            node.parent = parent;
    }

    private boolean colorOf(RBNode<T> node) { //获得节点的颜色
        return node != null ? node.color : BLACK;
    }

    private boolean isRed(RBNode<T> node) { //判断节点的颜色
    ...
    private void setRed(RBNode<T> node) { //设置节点的颜色
        if (node != null)
            node.color = RED;
    }
